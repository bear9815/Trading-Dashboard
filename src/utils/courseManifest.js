...
function normalizeLesson(rawLesson = {}, index = 0) {
  ...
  return {
    ...
    transcriptStatus: typeof rawLesson.transcriptStatus === 'string' ? rawLesson.transcriptStatus.trim() : (transcriptText ? 'ready' : 'pending'),
    enrichmentStatus: typeof rawLesson.enrichmentStatus === 'string' ? rawLesson.enrichmentStatus.trim() : 'pending',
    enrichmentError: String(rawLesson.enrichmentError || '').trim(),
