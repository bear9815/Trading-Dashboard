...
assert.equal(result.selectionProfile.focusZone.label, winningBucket.label)
assert.equal(result.selectionProfile.focusZone.avgR, winningBucket.avgR)
assert.ok(result.selectionProfile.avoidZones.some(zone => zone.bucketKey === losingBucket.key))
assert.equal(result.selectionProfile.bestSetup.key, 'positive_rising')
assert.equal(result.selectionProfile.weakestSetup.key, 'negative_weakening')
assert.equal(result.selectionProfile.signalPreference.key, 'above_signal')
assert.equal(result.selectionProfile.lifecyclePreference.key, 'held_above_signal')
assert.ok(result.selectionProfile.notes.some(note => note.includes('best entry z bucket')))
assert.ok(result.selectionProfile.notes.some(note => note.includes('low sample')))
